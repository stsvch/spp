...
export default function TaskForm({ mode = "create" }) {
  ...
  async function handleSubmit(e) {
    ...
    setError("");
    if (!canManage) {
      setError("Недостаточно прав. Задачи могут создавать/редактировать администраторы или участники проекта.");
      return;
    }
  ...
  const pageTitle = mode === "edit" ? "Редактировать задачу" : "Новая задача";
    ...
    <section>
      ...
      <form className="form form-wide" onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-actions">
          <button type="submit" disabled={!canManage}>{mode === "edit" ? "Сохранить" : "Создать"}</button>
        </div>
      </form>
    </section>
  );
